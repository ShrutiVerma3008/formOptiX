...
def run_lp_optimizer(df_schedule, monthly_budget_cr=8.0):
    ...
    if PULP_AVAILABLE:
        ...
        # Fallback: just-in-time heuristic
        opt_buy_w = [max(0, demand_w[t] - (0 if t==0 else max(0, opt_buy_w[t-1]-demand_w[t-1]))) for t in range(n_weeks)]
...
def make_cost_comparison(results):
    ...
    fig.update_yaxis(title_text="Cost (₹ Crore)")
    return fig
...
def make_inventory_curve(results, weeks):
    ...
    fig.update_xaxis(title_text="Project Week")
    fig.update_yaxis(title_text="Panel Count")
    return fig
...
def make_forecast_chart(weeks, demand, forecast, upper, lower):
    ...
    fig.update_xaxis(title_text="Week")
    fig.update_yaxis(title_text="Panel Count")
    return fig
...
def make_cluster_chart(df_floors):
    ...
    fig.update_xaxis(title_text="Slab Area (sqm)")
    fig.update_yaxis(title_text="Wall Length (m)")
    return fig
...
if run_btn:
    # ── Generate data
    with st.spinner("🏗️  Generating building data..."):
...
if st.session_state.results_ready:
    ...
    with tab1:
        ...
        fig_dfi.update_yaxis(range=[0, 110], title_text="Repetition Score (%)")
    ...
    with tab4:
        ...
        st.dataframe(
            ...
            .applymap(lambda v: f"color: {ORANGE}; font-weight:bold;" if isinstance(v, str) and v == "Typical" else "")
